...
USE `curbspringsdb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `curbspringsdb`.`admin_view` AS select `curbspringsdb`.`reservation`.`user_id` AS `user_id`,`curbspringsdb`.`parkingspot`.`spot_owner_id` AS `spot_owner_id`,`curbspringsdb`.`reservation`.`start_time` AS `start_time`,`curbspringsdb`.`reservation`.`end_time` AS `end_time`,`curbspringsdb`.`reservation`.`status` AS `status`,`curbspringsdb`.`parkingspot`.`address` AS `address`,`curbspringsdb`.`vehicle`.`license_plate` AS `license_plate`,`curbspringsdb`.`vehicle`.`vehicle_type` AS `vehicle_type` from ((`curbspringsdb`.`reservation` join `curbspringsdb`.`parkingspot` on((`curbspringsdb`.`parkingspot`.`spot_id` = `curbspringsdb`.`reservation`.`spot_id`))) join `curbspringsdb`.`vehicle` on((`curbspringsdb`.`vehicle`.`license_plate` = `curbspringsdb`.`reservation`.`license_plate`)));
...
USE `curbspringsdb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `curbspringsdb`.`spot_owner_view` AS select `curbspringsdb`.`reservation`.`start_time` AS `start_time`,`curbspringsdb`.`reservation`.`end_time` AS `end_time`,`curbspringsdb`.`reservation`.`status` AS `status`,`curbspringsdb`.`parkingspot`.`address` AS `address`,`curbspringsdb`.`vehicle`.`license_plate` AS `license_plate`,`curbspringsdb`.`vehicle`.`vehicle_type` AS `vehicle_type` from ((`curbspringsdb`.`reservation` join `curbspringsdb`.`parkingspot` on((`curbspringsdb`.`parkingspot`.`spot_id` = `curbspringsdb`.`reservation`.`spot_id`))) join `curbspringsdb`.`vehicle` on((`curbspringsdb`.`vehicle`.`license_plate` = `curbspringsdb`.`reservation`.`license_plate`))) where (`curbspringsdb`.`parkingspot`.`spot_owner_id` = 4);
...
USE `curbspringsdb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `curbspringsdb`.`user_view` AS select `curbspringsdb`.`reservation`.`start_time` AS `start_time`,`curbspringsdb`.`reservation`.`end_time` AS `end_time`,`curbspringsdb`.`reservation`.`status` AS `status`,`curbspringsdb`.`parkingspot`.`address` AS `address`,`curbspringsdb`.`vehicle`.`license_plate` AS `license_plate`,`curbspringsdb`.`vehicle`.`vehicle_type` AS `vehicle_type` from ((`curbspringsdb`.`reservation` join `curbspringsdb`.`parkingspot` on((`curbspringsdb`.`parkingspot`.`spot_id` = `curbspringsdb`.`reservation`.`spot_id`))) join `curbspringsdb`.`vehicle` on((`curbspringsdb`.`vehicle`.`license_plate` = `curbspringsdb`.`reservation`.`license_plate`))) where (`curbspringsdb`.`reservation`.`user_id` = 1);
